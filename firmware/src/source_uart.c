...
static bool reset_and_get_banner(boot_event_t *ev_out)
{
    ...
    *ev_out = (boot_event_t){ .klass = BOOT_TAG_NONE };
    ...
    ESP_LOGI(TAG, "Phase 2: RPI-RF-MOD-RST GPIO%d active-HIGH (HM-MOD held in reset)",
             PIN_RST_RPIRF);
    // 3000 ms wait: RPI-RF-MOD-Banner kommt typisch nach ~800ms, aber unter
    // Last (z.B. nach vielen CSMA/CA-init-Runden in App-Mode + harten RST)
    // empirisch beobachtet bis ~2200 ms.  Mit 1500 ms timeout'ten viele
    // pulse_rst_only-Aufrufe in Folge — Symptom: uart.tag=='' + boot=False
    // nach 50+ cmd=4-Cycles, obwohl Modul intern weiterläuft (frames_ok
    // steigt).  3000 ms gibt sauberen Headroom (v0.20-Pattern für
    // CHANGE_APP-Push war ähnlich: 1500→3500 ms).
    if (try_reset_get_banner(PIN_RST_RPIRF, POL_RPI_RF, 3000, ev_out)) {
...
esp_err_t source_uart_pulse_rst_only(char *tag_out, size_t tag_cap)
{
    ...
    // Modul-State markieren: nach Pulse ist's im BL, aber app_tag NICHT
    // pre-emptiv leeren — wenn der Banner-wait timeout schlägt (z.B.
    // unter Last bei vielen cmd=4 in Folge), behalten wir den letzten
    // bekannten Tag statt einer leeren Anzeige.  Sonst kumulieren
    // failed Probes zu uart.tag=='' + boot=False obwohl das Modul real
    // weiterläuft (Bug-Symptom 2026-05-11).
    S.boot_done = false;
    ...
    if (!got) {
        ESP_LOGE(TAG, "pulse_rst_only: kein Banner-Tag empfangen — app_tag '%s' bleibt erhalten",
                 S.app_tag);
        return ESP_FAIL;
    }
